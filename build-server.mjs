...
async function sessionToken(secret){const key=await crypto.subtle.importKey('raw',bytes.encode(secret),'HMAC',{name:'HMAC',hash:'SHA-256'},false,['sign']);const sig=await crypto.subtle.sign('HMAC',key,bytes.encode('green-fn-dynasty'));return [...new Uint8Array(sig)].map(x=>x.toString(16).padStart(2,'0')).join('')}
